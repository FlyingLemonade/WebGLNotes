...
function main() {
  ...
  var shader_fragment_source = `
    ...
    //Light source
    const vec3 source_ambient_color = vec3(1., 1., 0.);
    ...
    const vec3 view_direction = vec3(0., -4., -20.); //Lihat di translate y z dibawah
    ...
    void main(void) {
      ...
      vec3 I_diffuse = source_diffuse_color * mat_diffuse_color * dot(source_direction, vNormal);
      vec3 R = reflect(source_direction, vNormal);
      vec3 V = normalize(view_direction);
      vec3 I_specular = source_specular_color * mat_specular_color * pow(max(0., dot(R, V)), 10.); 
